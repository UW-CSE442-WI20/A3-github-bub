...
var sqMinEvtCt, sqMaxEvtCt;
var sqMinWk, sqMaxWk;
var sqMinMonth, sqMaxMonth;
var sqDim, sqPad;
var sqN, sqM;
var sqZoomInDur, sqZoomOutDur;
var sqLabelSize;
var sqPltWidth, sqPltHeight;
var sqFillExp = 2;
var sqFillColors = ['#ebedf0', '#c6e48b', '#7bc96f', '#239a3b', '#196127'];
var squares;

function getSqStats() {
  sqMinEvtCt = d3.min(sqDataset, function (d) {
    return d[3];
  });
  sqMaxEvtCt = d3.max(sqDataset, function (d) {
    return d[3];
  });
  sqMinWk = d3.min(sqDataset, function (d) {
    return d[1];
  });
  sqMaxWk = d3.max(sqDataset, function (d) {
    return d[1];
  });
  sqMinMonth = d3.min(sqDataset, function (d) {
    return d[4];
  });
  sqMaxMonth = d3.max(sqDataset, function (d) {
    return d[4];
  }); // square dimensions

  sqDim = 12;
  sqPad = 4; // square counts

  sqN = sqMaxWk - sqMinWk + 1;
  sqM = 7; // square size transition timings

  sqZoomInDur = 100; // ms

  sqZoomOutDur = 100; // circle transition timings

  cirZoomInDur = 50;
  cirSize = 1.5;
  cirZoomSize = 2.5; // label properties

  sqLabelSize = sqDim;
  sqLabelFadeInDur = 200;
  sqLabelFadeOutDur = sqLabelFadeInDur;
  sqLabelOpacity = 0.6; // square plot svg dimensions
...
function title() {
  d3.select('#title').style('width', sqPltWidth * 3 / 4 + 'px');
}

function sqPltCaption() {
  caption = d3.select('body').append('div').attr('id', 'sqpltcaption');
  caption.style('width', sqPltWidth * 3 / 4 + 'px').append('p').text('Total GitHub event counts are plotted over the past year, \
...
function scatterTitle() {
  title = d3.select('body').append('div').attr('id', 'scatterTitle').style('width', sqPltWidth * 3 / 4 + 'px').append('h2').text('TOTAL EVENTS PER DAY');
}

function evtctStatsTitle() {
  title = d3.select('body').append('div').attr('class', 'statsTitle');
  title.style('width', sqPltWidth * 3 / 4 + 'px').append('h2').text('ORG-OWNED VS. INDIVIDUALLY-OWNED REPO EVENTS');
}

function repoStatsTitle() {
  title = d3.select('body').append('div').attr('class', 'statsTitle'); // title.append('hr')

  title.style('width', sqPltWidth * 3 / 4 + 'px').append('h2').text('TOP REPO');
}

function userStatsTitle() {
  title = d3.select('body').append('div').attr('class', 'statsTitle'); // title.append('hr')

  title.style('width', sqPltWidth * 3 / 4 + 'px').append('h2').text('TOP USER');
}

function clearBoxSelection() {
  d3.selectAll('.selected').classed('selected', false).transition().duration(sqZoomInDur).attr('x', function (d) {
    return squareXPos(d);
  }).attr('y', function (d) {
    return squareYPos(d);
  }).attr('width', sqDim).attr('height', sqDim).style('stroke', 'none');
}

function squareplot() {
  var plt = d3.select('body').append('svg').attr('id', 'squareplot').attr('width', sqPltWidth).attr('height', sqPltHeight);
  addEvents();
  plt.append('text').classed('daylabel', true).attr('x', sqPad + sqPltWidth / 8).attr('y', sqPltHeight - sqPad); // squares

  squares = plt.selectAll('rect').data(sqDataset).enter().append('rect').attr('x', function (d) {
    return squareXPos(d);
  }).attr('y', function (d) {
    return squareYPos(d);
  }).attr('width', sqDim).attr('height', sqDim).attr('fill', function (d) {
    return squareFill(d);
  }).on('click', function (d) {
    return console.log(d);
  }).on('mouseover', function (d) {
    plt.select('.daylabel').transition().duration(sqLabelFadeInDur).style('fill-opacity', sqLabelOpacity).text(dayOfMonthLabel(d));
  }).on('mouseout', function () {
    plt.select('.daylabel').transition().duration(sqLabelFadeOutDur).style('fill-opacity', 0);
  }); // month labels

  plt.selectAll('text').data(sqDataset).enter().append('text').text(function (d) {
    return sqMonthLabel(d);
  }).attr('x', function (d) {
    return squareXPos(d);
  }).attr('y', sqLabelSize); // day labels

  plt.append('text').text('Mon').attr('x', sqPad).attr('y', sqLabelSize + squareYPos([0, 0, 2, 0, 0, 0, 0, 0]));
  plt.append('text').text('Wed').attr('x', sqPad).attr('y', sqLabelSize + squareYPos([0, 0, 4, 0, 0, 0, 0, 0]));
  plt.append('text').text('Fri').attr('x', sqPad).attr('y', sqLabelSize + squareYPos([0, 0, 6, 0, 0, 0, 0, 0]));
  legend = plt.append('g').attr('transform', "translate(".concat(sqPltWidth * 6 / 8, ", ").concat(sqPltHeight - sqPad, ")")); // legend

  legend.append('text').text('less events').attr('x', -2 * sqPad).attr('text-anchor', 'end');
  legend.append('text').text('more events').attr('x', 2 * sqPad + 5 * (sqDim + sqPad) + sqPad);
  legend.selectAll('rect').data(sqFillColors).enter().append('rect').attr('x', function (d, i) {
    return sqPad + i * (sqDim + sqPad);
  }).attr('y', -sqLabelSize).attr('width', sqDim).attr('height', sqDim).attr('fill', function (d) {
    return d;
  });
}

function addEvents() {
  d3.select('#squareplot').on('mousedown', function () {
    ...
    d3.selectAll('#squareplot>rect').each(function (d, i) {
      ...
      if (squareXPos(d) <= Math.max(selectBoxX, curX) && Math.min(curX, selectBoxX) <= squareXPos(d) + sqDim && squareYPos(d) <= Math.max(selectBoxY, curY) && Math.min(curY, selectBoxY) <= squareYPos(d) + sqDim) {
        // already selected
        ...
        d3.select(this).moveToFront().transition().duration(sqZoomOutDur).attr('x', squareXPos(d) + sqDim / 8).attr('y', squareYPos(d) + sqDim / 8).attr('width', sqDim * 3 / 4).attr('height', sqDim * 3 / 4).style('stroke', 'orange').style('stroke-weight', sqDim / 4 + "px").style('style-dasharray', '5, 5');
        d3.select(this).classed('selecting', true);
      } else {
        // deselect it
        d3.select(this).transition().duration(sqZoomInDur).attr('x', squareXPos(d)).attr('y', squareYPos(d)).attr('width', sqDim).attr('height', sqDim).style('stroke', 'none');
...
function highlightSelectedScatterPlot() {
  d3.selectAll('g circle').transition().duration(cirZoomInDur).style('fill', function (d, i) {
    return checkIfCircleSelected(d, i) ? "orange" : sqFillColors[4];
  }).attr('r', function (d, i) {
    return checkIfCircleSelected(d, i) ? cirZoomSize : cirSize;
  });
}

function checkIfCircleSelected(d, i) {
  return squares._groups[0][i].className.baseVal == 'selected' || squares._groups[0][i].className.baseVal == 'selecting';
}
...
function squareFill(d) {
  var sqFillScale = d3.scaleQuantize().domain([Math.pow(sqMinEvtCt, sqFillExp), Math.pow(sqMaxEvtCt, sqFillExp)]).range(sqFillColors);
  return sqFillScale(Math.pow(d[3], sqFillExp));
}
...
function dayOfMonthLabel(d) {
  // return d
  return "".concat(months[d[4] - 1], " ").concat(d[5]).concat(d[5] % 10 == 0 || d[5] >= 11 && d[5] <= 19 ? 'th' : daySuffix[Math.min(3, d[5] % 10 - 1)], " ").concat(d[7]);
}
...
var lnMinDay, lnMaxDay;
var lnPltWidth, lnPltHeight;
var lnPltPad;
var lnXMap, lnYMap;

function getLnStats() {
  lnMinDay = d3.min(lnDataset, function (d) {
    return d[0];
  });
  lnMaxDay = d3.max(lnDataset, function (d) {
    return d[0];
  ...
  lnPltPad = sqPad; // x axis data map

  lnXMap = d3.scaleLinear().domain([lnMinDay, lnMaxDay]).range([0 + lnPltPad + lnPltAxisSize, lnPltWidth - lnPltPad]); // y axis data map

  lnYMap = d3.scaleLinear().domain([d3.min(lnDataset, function (d) {
    return d[3];
  }), d3.max(lnDataset, function (d) {
    return d[3];
...
function scatterplot() {
  // make svg
  d3.select('body').append('svg').attr('width', lnPltWidth).attr('height', lnPltHeight).attr('align', 'center').attr('id', 'scatterplot').append('g').attr('id', 'scatter');
  var xScale = d3.scaleTime().domain([new Date(new Date(lnDataset[0][7], 0).setDate(lnMinDay)), new Date(new Date(lnDataset[0][7], 0).setDate(lnMaxDay))]).range([0 + lnPltPad + lnPltAxisSize, lnPltWidth - lnPltPad]);
  var yScale = d3.scaleLinear().domain([d3.min(lnDataset, function (d) {
    return d[3];
  }), d3.max(lnDataset, function (d) {
    return d[3];
  })]).range([lnPltHeight - lnPltPad - lnPltAxisSize, 0 + lnPltPad]);
  d3.select('#scatterplot').append('g').attr('transform', "translate(0, ".concat(lnPltHeight - lnPltAxisSize, ")")).call(d3.axisBottom().scale(xScale).tickFormat(d3.timeFormat("%b")));
  d3.select('#scatterplot').append('g').attr('transform', "translate(".concat(lnPltAxisSize, ", 0)")).call(d3.axisLeft().scale(yScale).tickFormat(format));
  redrawScatterPlot();
}
...
function redrawScatterPlot() {
  ...
  d3.selectAll('#scatter').selectAll('dot').data(lnDataset).enter().append('circle').attr('cx', function (d) {
    return lnXMap(d[0]);
  }).attr('cy', function (d) {
    return lnYMap(d[3]);
  }).attr('r', cirSize).style('fill', sqFillColors[4]);
}

var barWidth, barHeight;
var barDuration = 400;
var barEase = d3.easeExp;
var barLabelPad = 12;
var barLabelSize = 12;
var barPctLabelSize = 10;
var org_pct, ind_pct;
var org_ct, ind_ct;
var org_change_pct, ind_change_pct;
var tot_glob_ct, org_glob_ct, ind_glob_ct;

function evtctStats() {
  tot_glob_ct = d3.mean(sqDataset, function (d) {
    return d[3];
  });
  org_glob_ct = d3.mean(sqDataset, function (d) {
    return d[8];
  });
  ind_glob_ct = d3.mean(sqDataset, function (d) {
    return d[9];
  });
  barWidth = sqPltWidth * 3 / 4;
  barHeight = barLabelPad + 2 * (barLabelSize + barLabelPad);
  var svg = d3.select('body').append('svg').attr('id', 'eventcountstats').attr('width', barWidth).attr('height', barHeight);
  var midpoint = barWidth / 2;
  svg.append('rect').attr('id', 'org_rect').attr('x', 0).attr('y', 0).attr('width', midpoint).attr('height', barHeight).attr('fill', '#305da1');
  svg.append('rect').attr('id', 'ind_rect').attr('x', midpoint).attr('y', 0).attr('width', barWidth - midpoint).attr('height', barHeight).attr('fill', sqFillColors[2]);
  svg.append('text').attr('id', 'org_label').attr('x', barLabelPad).attr('y', barLabelPad + barLabelSize).attr('fill', 'white').text('ORG').style('font-size', barLabelSize + 'pt');
  svg.append('text').attr('id', 'ind_label').attr('x', barWidth - barLabelPad).attr('y', barLabelPad + barLabelSize).attr('text-anchor', 'end').attr('fill', 'white').text('IND').style('font-size', barLabelSize + 'pt');
  org_pct = 50;
  svg.append('text').attr('id', 'org_pct').attr('x', midpoint - barLabelPad).attr('y', barHeight - barLabelPad).attr('text-anchor', 'end').attr('fill', 'white').text(org_pct.toFixed(0)).style('font-size', 2 * barLabelSize + barLabelPad + 'pt');
  ind_pct = 50;
  svg.append('text').attr('id', 'ind_pct').attr('x', midpoint + barLabelPad).attr('y', barHeight - barLabelPad).attr('fill', 'white').text(ind_pct.toFixed(0) + '%').style('font-size', 2 * barLabelSize + barLabelPad + 'pt');
  org_ct = 0;
  org_change_pct = 0;
  svg.append('text').attr('id', 'org_evt_ct').attr('x', barLabelPad).attr('y', barHeight - barLabelPad).attr('fill', 'white').text(org_ct).style('font-size', barPctLabelSize + 'pt');
  ind_ct = 0;
  ind_change_pct = 0;
  svg.append('text').attr('id', 'ind_evt_ct').attr('x', barWidth - barLabelPad).attr('y', barHeight - barLabelPad).attr('text-anchor', 'end').attr('fill', 'white').text(ind_ct).style('font-size', barPctLabelSize + 'pt');
  redrawEventCountStats();
}
...
function redrawEventCountStats() {
  ...
  if (d3.selectAll('.selected')._groups[0].length == 0) {
    ...
    // selected average
    total = d3.mean(d3.selectAll('.selected').data(), function (d) {
      return d[3];
    });
    org = d3.mean(d3.selectAll('.selected').data(), function (d) {
      return d[8];
    });
    ind = d3.mean(d3.selectAll('.selected').data(), function (d) {
      return d[9];
    });
  ...
  var midpoint = barWidth * org / total;
  org_pct_new = org / total * 100;
  ind_pct_new = ind / total * 100;
  d3.select('#org_rect').transition().duration(barDuration).ease(barEase).attr('width', midpoint);
  d3.select('#ind_rect').transition().duration(barDuration).ease(barEase).attr('x', midpoint).attr('width', barWidth - midpoint);
  d3.select('#org_pct').transition().duration(barDuration).ease(barEase).textTween(function () {
    var i = d3.interpolate(org_pct, org_pct_new);
    return function (t) {
      return "".concat((org_pct = i(t)).toFixed(0));
    };
  }).attr('x', midpoint - barLabelPad);
  d3.select('#ind_pct').transition().duration(barDuration).ease(barEase).textTween(function () {
    var i = d3.interpolate(ind_pct, ind_pct_new);
    return function (t) {
      return "".concat((ind_pct = i(t)).toFixed(0));
    };
  }).attr('x', midpoint + barLabelPad);
  d3.select('#org_evt_ct').transition().duration(barDuration).ease(barEase).textTween(function () {
    var i = d3.interpolate(org_ct, org);
    var j = d3.interpolate(org_change_pct, org_change_pct_new);
    return function (t) {
      return "".concat(format(org_ct = i(t)), " (").concat((j(t) > 0 ? '+' : '') + (org_change_pct = j(t)).toFixed(1), "%)");
    };
  });
  d3.select('#ind_evt_ct').transition().duration(barDuration).ease(barEase).textTween(function () {
    var i = d3.interpolate(ind_ct, ind);
    var j = d3.interpolate(ind_change_pct, ind_change_pct_new);
    return function (t) {
      return "(".concat((j(t) > 0 ? '+' : '') + (ind_change_pct = j(t)).toFixed(1), "%) ").concat(format(ind_ct = i(t)));
    };
  });
}

format = d3.format(".3s");
var maxRepoCtGlob, maxRepoGlob;

function repoStats() {
  maxRepoCtGlob = d3.max(sqDataset, function (d) {
    return d[10];
  });
  maxRepoGlob = sqDataset[sqindexof(sqDataset, maxRepoCtGlob, 10)][12];
  var div = d3.select('body').append('div').attr('id', 'repostats').style('width', barWidth + 'px');
  div.append('a');
  div.append('p');
  redrawRepoStats();
}
...
function redrawRepoStats() {
  ...
  if (d3.selectAll('.selected')._groups[0].length == 0) {
    ...
    // selected average
    var selection = d3.selectAll('.selected').data();
    maxRepoCt = d3.max(selection, function (d) {
      return d[10];
    });
    maxRepo = selection[sqindexof(selection, maxRepoCt, 10)][12];
  }

  d3.select('#repostats>a').attr('href', 'https://www.github.com/' + maxRepo).text(maxRepo);
  d3.select('#repostats>p').text("".concat(format(maxRepoCt), " stars"));
}

var maxUserCtGlob, maxUserGlob;

function userStats() {
  maxUserCtGlob = d3.max(sqDataset, function (d) {
    return d[11];
  });
  maxUserGlob = sqDataset[sqindexof(sqDataset, maxUserCtGlob, 11)][13];
  var div = d3.select('body').append('div').attr('id', 'userstats').style('width', barWidth + 'px');
  div.append('a');
  div.append('p');
  redrawUserStats();
}
...
function redrawUserStats() {
  ...
  if (d3.selectAll('.selected')._groups[0].length == 0) {
    ...
    // selected average
    var selection = d3.selectAll('.selected').data();
    maxUserCt = d3.max(selection, function (d) {
      return d[11];
    });
    maxUser = selection[sqindexof(selection, maxUserCt, 11)][13];
  }

  d3.select('#userstats>a').attr('href', 'https://www.github.com/' + maxUser).text(maxUser);
  d3.select('#userstats>p').text("".concat(maxUserCt, " commit comments"));
}
...
function footer() {
  footer = d3.select('body').append('div').attr('id', 'footer');
  footer.style('width', sqPltWidth * 3 / 4 + 'px').append('a').attr('href', 'https://github.com/UW-CSE442-WI20/A3-github-bub/blob/master/writeup.txt').text('writeup');
}
},{"d3":"../node_modules/d3/index.js","../queries/events_per_day_2019_org_union_sorted_day.csv":"../queries/events_per_day_2019_org_union_sorted_day.csv"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
